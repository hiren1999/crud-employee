...
module.exports = async function validateRegisterInput(data, url = '') {
    let errors = {};
    ...
    if (!Validator.isLength(data.mobileno, { max: 10 })) {
        errors.mobileno = 'Mobile must be 10 numbers';
    }
    if (!Validator.isNumeric(data.mobileno)) {
    ...
    // PROFILE PHOTO FEILD
    if (url != 'edit') {
        if (Validator.isEmpty(data.profilephoto)) {
            errors.profilephoto = 'Please upload Profile Image';
        }
    }
